...
# S3_BUCKET_NAME is passed as an environment variable from Jenkins or set manually for local execution.
# S3_BUCKET_NAME="<YOUR_S3_BUCKET_NAME>" # Uncomment and set if running manually outside Jenkins
...
docker run --rm \
    ...
    /bin/bash -c " \
        ...
        find /${LAYER_DIR}/${PYTHON_VERSION}/lib/${PYTHON_VERSION}/site-packages/ -type f -name '*.pyc' -delete && \
